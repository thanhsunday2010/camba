...
export function MascotHero({
  ...
  return (
    <div className={cn("flex w-full min-w-0 max-w-full flex-col items-center gap-4 sm:flex-row sm:items-end", className)}>
      ...
      />
      <div className="relative w-full max-w-[min(100%,18rem)] rounded-3xl rounded-bl-sm border-2 border-white/30 bg-white/15 px-4 py-3 shadow-lg backdrop-blur-md sm:max-w-sm sm:px-5 sm:py-4">
        {messageSlot ?? (
          <p className="text-base font-extrabold leading-snug text-white md:text-lg">{message}</p>
        )}
        <div className="absolute -left-2 bottom-6 hidden h-4 w-4 rotate-45 border-b-2 border-l-2 border-white/30 bg-white/15 sm:block" />
      </div>
    </div>
  );
}
