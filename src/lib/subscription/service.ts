...
export async function getDailyUsageSnapshot(userId: string) {
  ...
  return {
    ...
    aiGradingCount: usage.aiGradingCount,
    aiGradingLimit: plan.limits.dailyAiGrading,
...
export async function canUseAiGrading(userId: string): Promise<boolean> {
  const [usage, limits] = await Promise.all([
    getOrCreateDailyUsage(userId),
    getUserPlanLimits(userId),
  ]);
  return usage.aiGradingCount < limits.dailyAiGrading;
}

export async function getAiGradingRemaining(userId: string): Promise<number> {
  const [usage, limits] = await Promise.all([
    getOrCreateDailyUsage(userId),
    getUserPlanLimits(userId),
  ]);
  return Math.max(0, limits.dailyAiGrading - usage.aiGradingCount);
}

export async function recordAiGradingUsage(userId: string) {
  const allowed = await canUseAiGrading(userId);
  if (!allowed) {
    const limits = await getUserPlanLimits(userId);
    return {
      ok: false as const,
      error: `Đã hết ${limits.dailyAiGrading} lượt AI chấm sửa hôm nay. Nâng cấp gói để tiếp tục.`,
    };
  }
  ...
  await db.dailyUsage.upsert({
    where: { userId_date: { userId, date } },
    create: { userId, date, aiGradingCount: 1 },
    update: { aiGradingCount: { increment: 1 } },
  });
